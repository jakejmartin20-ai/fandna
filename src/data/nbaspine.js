...
// Three of the six unique questions used to echo spine slots (roots/wins/role). They are RE-SCENED
// here to a distinct NBA angle - wording only. Option values A-D are unchanged, so the scoring cells,
// self-land, and reachability are all identical; only what the taker reads changes.
const RESCENE = {
  nba_q1: {
    question: "What pulls me toward a team:",
    options: [
  ...
  nba_q11: {
    question: "With everything on the line, the one I want next to me:",
    options: [
...
// The module the quiz shows + scores for NBA: nba.js's questions filtered to the 6 unique, with the
// three echo questions re-scened (id/type/phase kept from nba.js; question + option labels swapped).
export const moduleQuestions = nbaModule
  .filter(q => MODULE_UNIQUE.includes(q.id))
  .map(q => RESCENE[q.id] ? { ...q, ...RESCENE[q.id] } : q);
